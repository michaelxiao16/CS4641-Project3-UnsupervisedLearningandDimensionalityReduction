...
if __name__ == '__main__':
    ...
    datasets = []
    dataset1_details = {
            'data': loader.AP_AnalyticsData(verbose=verbose, seed=seed),
            'name': 'admissions_predict',
            'readable_name': 'AP Analytics',
            'best_nn_params': {'NN__activation': ['logistic'], 'NN__alpha': [1.0],
                               'NN__hidden_layer_sizes': [(18, 18)], 'NN__learning_rate_init': [0.016]}
        }
    dataset2_details = {
            'data': loader.Attractiveness_AnalyticsData(verbose=verbose, seed=seed),
            'name': 'attractiveness_analytics',
            'readable_name': 'Attractiveness Analytics',
            'best_nn_params': {'NN__activation': ['logistic'], 'NN__alpha': [1.0],
                               'NN__hidden_layer_sizes': [(18, 18)], 'NN__learning_rate_init': [0.016]}
    }
    if args.dataset1:
