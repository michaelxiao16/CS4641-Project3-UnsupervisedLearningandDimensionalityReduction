    ...
    def build_train_test_split(self, test_size=0.3):
        if not self.training_x and not self.training_y and not self.testing_x and not self.testing_y:
            self.training_x, self.testing_x, self.training_y, self.testing_y = ms.train_test_split(
                self.features, self.classes, test_size=test_size, random_state=self._seed
            )
...
class AP_AnalyticsData(DataLoader):
    def __init__(self, path='data/AP_Analytics.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
...
class Attractiveness_AnalyticsData(DataLoader):
    def __init__(self, path='data/Attractiveness_Analytics.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)
    ...
    def data_name(self):
        return 'Attractiveness_Analytics'
