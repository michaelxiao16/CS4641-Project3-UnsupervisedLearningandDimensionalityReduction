...
def runEverything():
    ...
    if (fn is "AP_Analytics.csv"):
        plotAPClusters(x)
    else:# (fn is "Attractiveness_Analytics.csv"):
        print("ATTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTt")
        plotAttractivenessClusters(x, y)
    print(exit)


def plotAPClusters(x, y):
    fig2 = plt.figure(2)
    ax2 = Axes3D(fig2)
    labels = np.array(y)
    X = np.array(x)
    ax2.scatter(X[:, 0], X[:, 1], X[:, 5], c=labels.astype(np.float), edgecolor='k')
...
def plotAttractivenessClusters(x):
    ...
    labels = np.array(y)
    X = np.array(x)
    ax2.scatter(X[:, 0], X[:, 31], X[:, 38], c=labels.astype(np.float), edgecolor='k')
    ax2.set_xlabel('Makeup')
...
def classify(x, y, normx, normy, init, n_init, max_iter, norm):
    ...
    fitter = KMeans(n_clusters=4, init=init, n_init=n_init, max_iter=max_iter).fit(x)
    X = np.array(x)
